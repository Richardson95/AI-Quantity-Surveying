...
import {
  ...
  CheckCircle2, Building2, ArrowRight,
    ...
    <div v-if="filtered.length" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <div v-for="v in filtered" :key="v.id"
        ...
        <div class="mt-3 flex items-center gap-3 text-sm">
          <span class="inline-flex items-center gap-1 font-semibold text-warning"><Star class="h-4 w-4 fill-warning" /> {{ v.rating }}</span>
          <span class="text-brand-light">·</span>
          <span class="text-brand-muted">{{ v.reviews }} reviews</span>
          <span class="text-brand-light">·</span>
        ...
        <div class="mt-4 flex gap-2">
          ...
          <button v-if="store.isUnlocked(v.id)" class="btn-secondary btn-sm flex-1" disabled>
      ...
      <div class="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-card-hover sm:rounded-2xl">
        ...
        <div :class="['bg-gradient-to-br', store.categoryMeta(activeVendor.category)?.accent]" class="relative p-6 text-white">
          ...
          <div class="mt-4 flex flex-wrap gap-2 text-sm">
            <span class="inline-flex items-center gap-1 rounded-lg bg-white/15 px-2.5 py-1 font-semibold"><Star class="h-4 w-4 fill-white" /> {{ activeVendor.rating }}</span>
            <span class="inline-flex items-center gap-1 rounded-lg bg-white/15 px-2.5 py-1">{{ activeVendor.reviews }} reviews</span>
